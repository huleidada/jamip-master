...
class Plot(FinderSet):
    ...
    def __init__(self, path=None, soft:str='vasp', depth=1, **kwargs):
        self.plotter = None
        self.savedir = None
        self.figtype = '.png'
        self.depth = depth
    ...
    def multiplot(self, job:str, figsize=None, **kwargs):
        '''
        ...
        success = 0
        for path in self.stdin:
            self.plotter.set_axes([job], figsize=figsize)
...
class BasePlot:
    ...
    def set_axes(self, jobs:list, figsize=None, **kwargs):

        self.axes = None
        if len(jobs) > 1:
            rows, height, width = self.fast_mosaic(jobs)
            gs_kw = dict(width_ratios=width, height_ratios=height)#,wspace=wspace,hspace=hspace)
            figsize = kwargs.get('figsize', (10*sum(width),10*sum(height)))
            fig, axes = plt.subplot_mosaic(rows, gridspec_kw=gs_kw, figsize=figsize,
                                            sharex='col', sharey='row',**kwargs)#constrained_layout=True)
            self.axes = axes
        elif figsize != None:
    ...
    def set_style(self, job:str):
        ...
        if self.axes != None:
            return plt.subplot(self.axes[job])
        else:
            return plt.gcf()
...
class VaspPlot(BasePlot):
    ...
    def plot_band(self,path,kpath=None,source='eigenval',fname='band.png',**kwargs):
        ...
        kpath=kpath2list(bf.kpath.kpts)
        xkpt = bf.get_xkpt()
        print(len(xkpt))
    ...
    def plot_fat_band(self,path,ptype='base',proj='emax',kpath=None, linestype='-', alpha=0.75,
                      max_z:int=1,interpolation:int=2,source='procar',fname='fatband.png',**kwargs):
        '''
        ...
        kpath=kpath2list(pf.kpath.kpts)
        xkpt = pf.get_xkpt()
        
        # set x axis % 
        xticks = [0] + np.cumsum(pf.kpath.insert).tolist()
        xticks[-1] -= 1
        xticks = [xkpt[i] for i in xticks]
        for i in xticks:
            plt.axvline(i,c='black')
    ...
    def plot_dos(self, path, proj='edos', cutoff:float=0.003, swap_axes:bool=False,
        ...
        # spin & limit
        limit = g.limit
        _limit = -limit if 'spin' in df.dostype else 0
        self._plot_swap(plt, xlim=(g.emin,g.emax),ylim=(_limit,limit))

        # swap_axes %
        if not swap_axes:
            plt.axvline(c='r',linestyle='--')
            if 'spin' in df.dostype:
    ...
    def plot_tdm(self,path,kpath=None,realspace=False,source='eigenval',fname='tdm.png',**kwargs):
        '''
        ...
        bf = BandFinder(path).get_data_from_wavecar().get_dipole(realspace=realspace).remove_duplicates()
    ...
    def plot_unfolding(self,path,smear=False,cache=False,fname='unfold.png',**kwargs):
        """
        ...
        elif 'primcell' in kwargs and 'dim' in kwargs:
            primcell = kwargs['primcell']
            trans = np.array(dim).astype(int)
        else:
            import spglib
            print('Warning! No primcell input, try spglib! Result maybe error...')
            primcell = spglib.find_primitive(cell)
            trans = np.rint(np.dot(cell[0],np.linalg.inv(primcell[0])), dtype=int)
        print(trans)
        ...
        # spectral-weight %
        if cache and (path/'sw.npy').exists():
            sw = np.load(path/'sw.npy')
        else:
            sw = w.spectral_weight(G)
            if cache:
                np.save(path/'sw.npy',sw)

        # kpath-in %
        kpath,insert = GrepKpath.read_kpath(path)
        kpath = kpath2list(kpath)
        # get xkpt %
        rec_cell = np.linalg.inv(primcell[0])
        intrans = np.linalg.inv(trans)        
        delta = np.linalg.norm(np.dot(K[1:]-K[:-1], intrans) @ rec_cell, axis=1)
        ...
        if smear:
            ...
            sw = sw * plt.rcParams['lines.markersize']**2
            for i in range(w.nspin):
                for nb in range(w.nbands):
                    plt.scatter(xkpt,w.bands[i,:,nb,0]-fermi, s=sw[i,:,nb], c='b')    
        if fname != None: self.builder.save(fname)
        return plt
    ...
    def phband(self, path, npoints=51, fname='phband.png',**kwargs):
        '''
        ...
        kpath = [['\\Gamma', 'X', 'S', 'Y','\\Gamma', 'Z','U', 'R', 'T', 'Z',]] #手动设置K点路径 # yxy
        #kpath = [['\\Gamma', 'S', 'R','Z', 'T',]]
        # kpath = [['\\Gamma', 'X', 'S', 'Y','\\Gamma',]]
        # kpath = [['\\Gamma', 'Z', 'D', 'B', '\\Gamma',]]
        # kpath = [['\\Gamma', 'M', 'K', '\\Gamma', 'A', 'L', 'H', 'A']]
        # kpath = [['\\Gamma', 'X', 'M', '\\Gamma',]]
        paths = [[kpoint['Kpoints'][i] for i in kpath[0]]]
        ...
        for i, (d, f, c) in enumerate(zip(distances_scaled, phonon._band_structure.frequencies, phonon._band_structure.path_connections)):
            curves = plt.plot(d, f, c='royalblue')
...
class QEPlot(BasePlot):
    ...
    def plot_dos(self, path, ptype, swap_axes=False, cutoff=0.003, fname='dos.png', **kwargs):
        '''
        plot band-figure base on dos.xml
        '''
        from jamip.analysis.qe import DosFinder
        plt = self.set_style(ptype)
        g = globalvar.dos
        ...
        dos_energy = dos_energy-df.get_vbm()-g.scissor
        ...
        if ptype == 'tdos':
            if df.spin == 1:
                x,y = (dos_energy, dos) if swap_axes else (dos, dos_energy)
                plt.plot(x,y)
        elif ptype == 'pdos':
