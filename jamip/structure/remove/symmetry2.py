...
class EqualTools:
    ...
    def unique_generators(self):
        ...
        if self.arithmetic_number in (26,27,28,42,43,44) and self.group == 'layer':
            ...
            unique = ['2_001','-1']
        elif self.arithmetic_number in (9,) and self.group == 'layer':
            unique = ['2_001','-1']
    ...
    def set_lattice_symmetry(self, lattice):
        self._lattice_symmetry = EqualTools.from_lattice(lattice)
...
#        operations = self.all_operations if all_operations else self.operations
        ...
        for symbol,matrix in operations.items():
            ...
            shift = self.istranslate(coord, coord2, elements, lattice, tol=tol*10)
#            print(symbol, shift)
    ...
    def istranslate(self, positions1, positions2, elements=None, lattice=None, tol=0.1):
        ...
        allvectors = []
        for i in values:
            indices = np.where(elements==i)[0]
            diff = positions1[indices][:1,:] - positions2[indices]
            allvectors.append(normalized(diff))
        ...
        for vec in vectors:
            ...
            for i in values:
                indices = np.where(elements==i)[0]
                coord1 = lexsort(normalized(positions1[indices]))
                coord2 = lexsort(normalized(positions2[indices] + vec))
